...
const FY_META = {
  ...
  'May':      {fy:'FY5/2025',  fy_jp:'2025年5月期',  prev_fy_jp:'2024年5月期',  next_fy_jp:'2026年5月期'},
}

// MIXED cost/routing test: 5 expect-LIGHT (clean, incl. verified Capcom+TIS) + 5 expect-DEEP (hard).
const INPUTS = [
  // --- CLEAN bucket (expect LIGHT) ---
  {ticker:'9697', name:'Capcom', name_jp:'カプコン', size:'large', fy_hint:'March', announce:'2025-05-13', dir:'up', pct:'+7.50%', expect:'light'},
  {ticker:'3626', name:'TIS', name_jp:'TIS', size:'large', fy_hint:'March', announce:'2025-05-08', dir:'up', pct:'+8.92%', expect:'light'},
  {ticker:'4716', name:'Oracle Japan', name_jp:'日本オラクル', size:'large', fy_hint:'May', announce:'2025-06-27', dir:'down', pct:'-8.17%', expect:'light'},
  {ticker:'4768', name:'Otsuka Corp', name_jp:'大塚商会', size:'large', fy_hint:'December', announce:'2026-02-02', dir:'down', pct:'-1.41%', expect:'light'},
  {ticker:'9434', name:'SoftBank Corp', name_jp:'ソフトバンク', size:'large', fy_hint:'March', announce:'2025-05-08', dir:'down', pct:'-2.89%', expect:'light'},
  // --- HARD bucket (expect DEEP) ---
  {ticker:'9433', name:'KDDI', name_jp:'KDDI', size:'large', fy_hint:'March', announce:'2025-05-14', dir:'down', pct:'-2.89%', expect:'deep', locked:'stock_reason_tag=consensus_miss（純利益¥685.7Bが市場コンセンサス¥705.4B未達が主因。business_reason_tag=one-off_cost_rolloff、前期ミャンマー引当・設備減損の剥落が主因）'},
  {ticker:'3659', name:'Nexon', name_jp:'ネクソン', size:'large', fy_hint:'December', announce:'2026-02-12', dir:'down', pct:'-11.58%', expect:'deep'},
  {ticker:'4676', name:'Fuji Media Holdings', name_jp:'フジ・メディアHD', size:'large', fy_hint:'March', announce:'2025-05-16', dir:'down', pct:'-2.63%', expect:'deep'},
  {ticker:'4689', name:'LY Corporation', name_jp:'LYコーポレーション', size:'large', fy_hint:'March', announce:'2025-05-07', dir:'down', pct:'-5.81%', expect:'deep', locked:'business_reason_tag=one-off_cost_rolloff（前期減損の剥落 約¥34.5Bが主因。ValueCommerce支配喪失益は¥6,967mと小。¥43B等は誤り）'},
  {ticker:'4385', name:'Mercari', name_jp:'メルカリ', size:'large', fy_hint:'June', announce:'2025-08-05', dir:'down', pct:'-3.80%', expect:'deep'},
]
...
const results = await pipeline(
  ...
  (light, c) => {
    ...
    const route = esc ? 'deep' : 'light'
    const hit = (route === c.expect) ? 'OK' : `MISROUTE(exp=${c.expect})`
    log(`${c.ticker} ${c.name}: route=${route} expect=${c.expect} ${hit} ${light&&light.escalate_reason?('· '+light.escalate_reason):''}`)
    if(!esc) return {...light, _tier:'light', expect:c.expect, route_ok:(route===c.expect), stock_dir:c.dir, stock_pct:c.pct, name:c.name, name_jp:c.name_jp, size:c.size, fy_label:FY_META[c.fy_hint].fy}
    return agent(deepPrompt(c, light), {label:c.ticker, phase:'Deep', schema:DEEP_SCHEMA, model:'sonnet'})
      .then(r => r ? {...r, _tier:'deep', expect:c.expect, route_ok:(route===c.expect)} : {...light, _tier:'deep_failed', expect:c.expect, stock_dir:c.dir})
  }
)
...
const nMisroute = valid.filter(r=>r.route_ok===false).length
log(`ROUTING: ${nLight} light(cheap), ${nDeep} deep(grounded), ${nMisroute} misrouted`)
await agent(
  `Write this JSON to exactly c:/Users/Sahal Saeed/Documents/tempest_ai/quick/New_quick/new_quick/quite_change/data/quarterly/_test_mixed.json using Write (overwrite). Confirm path + count.\n\n${JSON.stringify({arch:'tiered-router-mixed', light:nLight, deep:nDeep, misrouted:nMisroute, companies}, null, 2)}`,
  {label:'write', phase:'Deep', model:'haiku'}
)
return {light:nLight, deep:nDeep, misrouted:nMisroute, routes: valid.map(r=>({t:r.ticker, route:r._tier, expect:r.expect, ok:r.route_ok}))}
