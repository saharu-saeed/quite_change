...
export const meta = {
  name: 'it-q4-single-costtest',
  description: 'Single-agent merged pipeline cost/quality test on 11 known-answer companies',
  phases: [{title: 'Research', detail: 'one agent/company: fetch + numbers + business + stock reason'}]
}
...
${STOCK_TAGS}
 ...
 ⚠️ KDDI型: 大型還元があっても株価が下落＝還元よりconsensus_miss等が勝ったと素直に。
...
phase('Research')
log(`Single-agent cost test — MODEL=${MODEL} — ${INPUTS.length} companies`)
...
await agent(
  `Write this JSON to exactly c:/Users/Sahal Saeed/Documents/tempest_ai/quick/New_quick/new_quick/quite_change/data/quarterly/_test11_single_${MODEL}.json using Write (overwrite). Confirm path + count.\n\n${JSON.stringify({model:MODEL, companies}, null, 2)}`,
